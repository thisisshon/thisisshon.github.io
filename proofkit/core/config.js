 ...
 */
/**
 * UNLOCK — the screen a returning reviewer meets when this browser already knows who they are and
 * only the session has lapsed. It asks for a PIN and nothing else: re-asking for an identity that
 * is already remembered is noise.
 *
 * Built on the SAME card as the access-key screen — same wordmark, same title and sub, same
 * Advanced footer, same alt-route buttons — because it is the same door, opened by someone the
 * browser has already met. It used to be a bare label, input and button in the popup's own
 * stylesheet, which is why it looked like a different product.
 *
 *   opts.name        who is being unlocked, shown as the subtitle
 *   opts.role        appended after a middle dot when present
 *   opts.onSubmit    (pin) => void
 *   opts.onBiometric optional; renders the Touch ID route
 */
export function buildUnlock(opts) {
  opts = opts || {};
  const esc = (t) => String(t == null ? '' : t).replace(/[&<>"]/g, (c) =>
    ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));
  const who = [opts.name, opts.role].filter(Boolean).join(' · ');
  ...
  el.innerHTML =
    '<div class="pk-access-card">' +
      ...
      '<h1 class="pk-access-title">' + esc(opts.title || 'Welcome Back') + '</h1>' +
      '<p class="pk-access-sub">' + esc(who || 'Enter your PIN to continue.') + '</p>' +
      '<input class="pk-unlock-input" type="password" inputmode="numeric" autocomplete="current-password" ' +
        'placeholder="PIN" aria-label="PIN">' +
      '<button type="button" class="pk-unlock-go">Unlock</button>' +
      '<div class="pk-access-err" hidden></div>' +
      (opts.onBiometric
        ? '<button type="button" class="pk-access-alt" data-alt="bio">Use Touch ID Instead</button>'
        : '') +
    '</div>';

  const q = (sel) => el.querySelector(sel);
  const input = q('.pk-unlock-input');
  const go = q('.pk-unlock-go');
  ...
  const submit = () => {
    ...
    setError('');
    if (opts.onSubmit) opts.onSubmit(pin);
  };
  go.addEventListener('click', submit);
  // Enter submits. A single-field form where Enter does nothing is a small, constant irritation.
  input.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); submit(); } });
  input.addEventListener('input', () => { if (err.textContent) setError(''); });

  const bio = q('.pk-access-alt[data-alt="bio"]');
  if (bio) bio.addEventListener('click', () => { if (opts.onBiometric) opts.onBiometric(); });

  return {
    el,
    focus: () => input.focus(),
    getValue: () => input.value.trim(),
    /* Mounted once and updated in place. Rebuilding the card on every render would take focus out
     * of the field mid-typing, which is exactly when it is being used. */
    setWho: (name, role) => {
      q('.pk-access-sub').textContent = [name, role].filter(Boolean).join(' · ') || 'Enter your PIN to continue.';
    },
    setBusy: (busy) => { input.disabled = !!busy; go.disabled = !!busy; el.classList.toggle('is-busy', !!busy); },
    setError,
    // Same contract as the access screen: shake, say why, then clear so the next attempt starts clean.
    reject: (message) => {
      input.classList.remove('is-wrong'); void input.offsetWidth; input.classList.add('is-wrong');
      setError(message || '');
      setTimeout(() => { input.value = ''; input.focus(); }, 380);
    },
    reset: () => { input.value = ''; input.disabled = false; go.disabled = false;
                   el.classList.remove('is-busy'); input.classList.remove('is-wrong'); setError(''); },
  };
}
